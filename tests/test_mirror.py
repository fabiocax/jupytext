...
@pytest.mark.parametrize('nb_file',
                         list_notebooks('ipynb_all', skip='many hash'))
...
def test_ipynb_to_percent(nb_file, no_jupytext_version_number):
    assert_conversion_same_as_mirror(nb_file, 'auto:percent',
                                     'ipynb_to_percent')


@pytest.mark.parametrize('nb_file', list_notebooks('ipynb_all', skip=''))
def test_ipynb_to_hydrogen(nb_file, no_jupytext_version_number):
    assert_conversion_same_as_mirror(nb_file, 'auto:hydrogen',
                                     'ipynb_to_hydrogen')
...
@pytest.mark.parametrize(
    ...
    list_notebooks(
        'ipynb_all',
        skip='html-demo|julia_functional_geometry|xcpp_by_quantstack'))
...
@pytest.mark.parametrize('nb_file',
                         list_notebooks(
                             'ipynb_py',
                             skip='(raw|hash|frozen|magic|html|164|long)'))
...
@pytest.mark.parametrize('nb_file',
                         list_notebooks('julia') + list_notebooks('python') +
...
def test_sphinx_to_ipynb(nb_file, no_jupytext_version_number):
    assert_conversion_same_as_mirror(nb_file, 'ipynb:sphinx',
                                     'sphinx_to_ipynb')
...
def test_ipynb_to_r_percent(nb_file, no_jupytext_version_number):
    assert_conversion_same_as_mirror(nb_file, '.low.r:percent',
                                     'ipynb_to_percent')
...
def test_ipynb_to_scheme_percent(nb_file, extension,
    ...
    assert_conversion_same_as_mirror(nb_file, '{}:percent'.format(extension),
                                     'ipynb_to_percent')
