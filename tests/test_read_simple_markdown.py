...
def test_markdown_cell_with_code_works(
    nb=new_notebook(cells=[new_markdown_cell("""```python
1 + 1
```""")])):
...
def test_markdown_cell_with_noeval_code_works(nb=new_notebook(
    cells=[new_markdown_cell("""```python .noeval
1 + 1
```""")])):
    text = jupytext.writes(nb, 'md')
    nb2 = jupytext.reads(text, 'md')
...
def test_update_metadata_filter(no_jupytext_version_number,
                                org="""---
jupyter:
  kernelspec:
    display_name: Python 3
...
def test_update_metadata_filter_2(no_jupytext_version_number,
                                  org="""---
jupyter:
  jupytext:
    notebook_metadata_filter: -extra
