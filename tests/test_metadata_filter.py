...
@pytest.mark.parametrize('metadata_filter_string,metadata_filter_dict',
                         [('all, -widgets,-varInspector', {
                          ...
                          ('ExecuteTime, autoscroll, -hide_output', {
                              'additional': ['ExecuteTime', 'autoscroll'],
                              'excluded': ['hide_output']
                          })])
def test_string_to_dict_conversion(metadata_filter_string,
                                   metadata_filter_dict):
    assert metadata_filter_as_dict(
...
def test_metadata_filter_as_dict():
    ...
    assert metadata_filter_as_dict({'excluded':
                                    'all'}) == metadata_filter_as_dict('-all')


def test_metadata_filter_default():
    assert filter_metadata(to_dict(['technical', 'user', 'preserve']), None,
                           '-technical') == to_dict(['user', 'preserve'])
    assert filter_metadata(to_dict(['technical', 'user', 'preserve']), None,
                           'preserve,-all') == to_dict(['preserve'])
...
def test_metadata_filter_user_overrides_default():
    assert filter_metadata(to_dict(['technical', 'user',
                                    'preserve']), 'all,-user',
                           '-technical') == to_dict(['technical', 'preserve'])
    assert filter_metadata(to_dict(['technical', 'user', 'preserve']),
                           'user,-all', 'preserve') == to_dict(['user'])
...
def test_filter_nested_metadata():
    ...
    assert filter_metadata(metadata, 'I', '-all') == {
        'I': {
            '1': {
                'a': 1,
                'b': 2
