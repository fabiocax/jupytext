...
if is_pandoc_available():
    JUPYTEXT_FORMATS.append(
        NotebookFormatDescription(format_name='pandoc',
                                  extension='.md',
                                  header_prefix='',
...
if is_myst_available():
    JUPYTEXT_FORMATS.extend([
        NotebookFormatDescription(format_name=MYST_FORMAT_NAME,
                                  extension=ext,
                                  header_prefix='',
                                  cell_reader_class=None,
                                  cell_exporter_class=None,
...
def get_format_implementation(ext, format_name=None):
    ...
    raise JupytextFormatError(
        "No format associated to extension '{}'".format(ext))
...
def guess_format(text, ext):
    ...
    if ext in _SCRIPT_EXTENSIONS:
        ...
        double_percent_and_space_re = re.compile(
            r'^{}( %%|%%)\s'.format(comment))
        ...
        parser = StringParser(
            language='R' if ext in ['.r', '.R'] else 'python')
        for line in lines:
...
def divine_format(text):
    ...
    for comment in ['', '#'] + _COMMENT_CHARS:
        ...
        ext = metadata.get('jupytext', {}).get('text_representation',
                                               {}).get('extension')
        if ext:
...
def check_file_version(notebook, source_path, outputs_path):
    ...
    if ext.endswith('.ipynb'):
        return
    version = notebook.metadata.get('jupytext',
    ...
    raise JupytextFormatError(
        ...
        "Please remove one or the other file.".format(
            os.path.basename(source_path), format_name, version, current,
            os.path.basename(outputs_path)))


def format_name_for_ext(metadata,
                        ext,
                        cm_default_formats=None,
...
def update_jupytext_formats_metadata(metadata, new_format):
    ...
    metadata.setdefault('jupytext',
                        {})['formats'] = short_form_multiple_formats(formats)
...
def rearrange_jupytext_metadata(metadata):
    ...
    if jupytext_metadata.get('text_representation',
                             {}).get('jupytext_version', '').startswith('0.'):
        formats = jupytext_metadata.get('formats')
        if formats:
            jupytext_metadata['formats'] = ','.join([
                '.' + fmt if fmt.rfind('.') > 0 else fmt
...
def long_form_one_format(jupytext_format,
                         metadata=None,
                         update=None,
                         auto_ext_requires_language_info=True):
    ...
    if jupytext_format.rfind(':') >= 0:
    ...
    elif not jupytext_format or '.' in jupytext_format or (
            '.' + jupytext_format) in NOTEBOOK_EXTENSIONS + ['.auto']:
        ext = jupytext_format
        ...
        raise JupytextFormatError(
            ...
            .format(jupytext_format, ', '.join(NOTEBOOK_EXTENSIONS),
    ...
    if ext == '.auto':
        ext = auto_ext_from_metadata(
            metadata) if metadata is not None else '.auto'
        if not ext:
...
_BINARY_FORMAT_OPTIONS = [
    'comment_magics', 'split_at_heading', 'rst2md', 'cell_metadata_json',
    'use_runtools'
]
...
def validate_one_format(jupytext_format):
    ...
    if 'format_name' in jupytext_format and jupytext_format[
            'format_name'] not in _VALID_FORMAT_NAMES:
        raise JupytextFormatError(
            '{} is not a valid format name. Please choose one of {}'.format(
                jupytext_format.get('format_name'),
                ', '.join(_VALID_FORMAT_NAMES)))
    ...
    if ext not in NOTEBOOK_EXTENSIONS + ['.auto']:
        raise JupytextFormatError(
            ...
            .format(ext, "', '".join(NOTEBOOK_EXTENSIONS + ['.auto'])))
...
def auto_ext_from_metadata(metadata):
    ...
    if auto_ext is None:
        language = metadata.get('kernelspec',
                                {}).get('language') or metadata.get(
                                    'jupytext', {}).get('main_language')
        if language:
            for ext in _SCRIPT_EXTENSIONS:
                if same_language(language,
                                 _SCRIPT_EXTENSIONS[ext]['language']):
                    auto_ext = ext
                    break
