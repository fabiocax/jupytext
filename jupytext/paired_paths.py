...
def base_path(main_path, fmt):
    ...
    if not fmt:
        ...
        if ext not in NOTEBOOK_EXTENSIONS:
            raise InconsistentPath(
                "'{}' is not a notebook. Supported extensions are '{}'.".
                format(main_path, "', '".join(NOTEBOOK_EXTENSIONS)))
        return base
    ...
    if suffix:
        if not base.endswith(suffix):
            raise InconsistentPath(
                u"Notebook name '{}' was expected to end with suffix '{}'".
                format(base, suffix))
    ...
    if prefix_file_name:
        if not notebook_file_name.startswith(prefix_file_name):
            raise InconsistentPath(
                u"Notebook name '{}' was expected to start with prefix '{}'".
    ...
    if prefix_dir:
        if not notebook_dir.endswith(prefix_dir):
            raise InconsistentPath(
                u"Notebook directory '{}' was expected to end with directory prefix '{}'"
    ...
    if main_path not in paths:
        raise InconsistentPath(
            ...
            u"Current format is '{}', and paired formats are '{}'.".format(
                "','".join(paths), main_path, short_form_one_format(fmt),
