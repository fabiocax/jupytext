...
_MAGIC_RE = {
    ...
    re.compile(r"^({0} |{0})*(%|%%|%%%)[a-zA-Z]".format(
        _SCRIPT_EXTENSIONS[ext]['comment']))
    for ext in _SCRIPT_EXTENSIONS
}
_MAGIC_FORCE_ESC_RE = {
    _SCRIPT_EXTENSIONS[ext]['language']:
    re.compile(r"^({0} |{0})*(%|%%|%%%)[a-zA-Z](.*){0}\s*escape".format(
        _SCRIPT_EXTENSIONS[ext]['comment']))
    for ext in _SCRIPT_EXTENSIONS
}
_MAGIC_NOT_ESC_RE = {
    _SCRIPT_EXTENSIONS[ext]['language']:
    re.compile(r"^({0} |{0})*(%|%%|%%%)[a-zA-Z](.*){0}\s*noescape".format(
        _SCRIPT_EXTENSIONS[ext]['comment']))
    for ext in _SCRIPT_EXTENSIONS
}
...
_MAGIC_FORCE_ESC_RE['rust'] = re.compile(
    r"^(// |//)*:[a-zA-Z](.*)//\s*noescape")

# C# magics start with '#!'
_MAGIC_RE['csharp'] = re.compile(r"^(// |//)*#![a-zA-Z]")
_MAGIC_FORCE_ESC_RE['csharp'] = re.compile(
    r"^(// |//)*#![a-zA-Z](.*)//\s*escape")
_MAGIC_FORCE_ESC_RE['csharp'] = re.compile(
    r"^(// |//)*#![a-zA-Z](.*)//\s*noescape")
...
def need_explicit_marker(source,
                         language='python',
                         global_escape_flag=True,
                         explicitly_code=True):
...
def comment_magic(source,
    ...
    for pos, line in enumerate(source):
        if not parser.is_quoted() and (next_is_magic or is_magic(
                line, language, global_escape_flag, explicitly_code)):
            source[pos] = _COMMENT[language] + ' ' + line
            next_is_magic = language == 'python' and _LINE_CONTINUATION_RE.match(
                line)
        parser.read_line(line)
    return source
...
_ESCAPED_CODE_START.update({
    ext: re.compile(r"^({0} |{0})*({0}|{0} )\+".format(
        _SCRIPT_EXTENSIONS[ext]['comment']))
    for ext in _SCRIPT_EXTENSIONS
})
