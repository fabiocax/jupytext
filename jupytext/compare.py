...
def _multilines(obj):
    try:
        ...
        return [
            line.rstrip() for line in json.dumps(
                obj, indent=True, sort_keys=True).splitlines()
        ]
...
def compare(actual, expected, return_diff=False):
    ...
    if actual != expected:
        diff = '\n'.join(
            difflib.unified_diff(_multilines(expected),
                                 _multilines(actual),
                                 'expected',
                                 'actual',
                                 lineterm=''))
...
def filtered_cell(cell, preserve_outputs, cell_metadata_filter):
    ...
    filtered = {
        ...
        cell.source,
        'metadata':
        filter_metadata(cell.metadata, cell_metadata_filter,
...
def compare_notebooks(notebook_actual,
                      notebook_expected,
                      fmt=None,
    ...
    allow_missing_markdown_cell_metadata = allow_expected_differences and format_name in [
        'sphinx', 'spin'
    ]
    allow_removed_final_blank_line = allow_expected_differences

    cell_metadata_filter = notebook_actual.get('jupytext',
                                               {}).get('cell_metadata_filter')

    if format_name == 'sphinx' and notebook_actual.cells and notebook_actual.cells[
            0].source == '%matplotlib inline':
    ...
    for i, ref_cell in enumerate(notebook_expected.cells, 1):
        ...
        ref_lines = [
            line for line in ref_cell.source.splitlines()
            ...
            if raise_on_first_difference:
                raise NotebookDifference(
                    "Unexpected cell type '{}' for {} cell #{}:\n{}".format(
                        test_cell.cell_type, ref_cell.cell_type, i,
                        ref_cell.source))
        ...
        if (ref_cell.cell_type == 'code' and not allow_missing_code_cell_metadata) or \
            ...
            if allow_filtered_cell_metadata:
                ref_metadata = {
                    ...
                    for key in ref_metadata if key not in _IGNORE_CELL_METADATA
                }
                test_metadata = {
        ...
        test_lines.extend([
            line for line in test_cell.source.splitlines()
            if not _BLANK_LINE.match(line)
        ])
        ...
        if not same_content(ref_cell.source, test_cell.source,
                            allow_removed_final_blank_line):
            if ref_cell.source != test_cell.source:
                if raise_on_first_difference:
                    diff = compare(test_cell.source,
                                   ref_cell.source,
                                   return_diff=True)
                    raise NotebookDifference(
                        "Cell content differ on {} cell #{}: {}".format(
    ...
    while True:
        try:
            test_cell = next(test_cell_iter)
            if raise_on_first_difference:
                raise NotebookDifference('Additional {} cell: {}'.format(
    ...
    try:
        ...
        if raise_on_first_difference:
            raise NotebookDifference("Notebook metadata differ: {}".format(
                str(error)))
        modified_metadata = True

    error = []
    if modified_cells:
        error.append('Cells {} differ ({}/{})'.format(
            ','.join([str(i) for i in modified_cells]), len(modified_cells),
            len(notebook_expected.cells)))
    if modified_cell_metadata:
        error.append("Cell metadata '{}' differ".format("', '".join(
            [str(i) for i in modified_cell_metadata])))
...
def test_round_trip_conversion(notebook,
                               fmt,
                               update,
                               allow_expected_differences=True,
