...
def preferred_format(incomplete_format, preferred_formats):
    ...
    for fmt in long_form_multiple_formats(preferred_formats):
        if ((incomplete_format['extension'] == fmt['extension'] or
             (fmt['extension'] == '.auto' and incomplete_format['extension']
              not in ['.md', '.markdown', '.Rmd', '.ipynb']))
                and incomplete_format.get('suffix') == fmt.get(
                    'suffix', incomplete_format.get('suffix'))
                and incomplete_format.get('prefix') == fmt.get(
                    'prefix', incomplete_format.get('prefix'))):
...
def build_jupytext_contents_manager_class(base_contents_manager_class):
    ...
    class JupytextContentsManager(base_contents_manager_class, Configurable):
        ...
        default_notebook_metadata_filter = Unicode(
            u'',
            help=
            "Cell metadata that should be save in the text representations. "
        ...
        outdated_text_notebook_margin = Float(
            1.0,
            help=
            'Refuse to overwrite inputs of a ipynb notebooks with those of a '
        ...
        def update_paired_notebooks(self, path, fmt, formats):
            ...
            if len(long_formats) == 1 and set(
                    long_formats[0]) <= {'extension'}:
                return

            short_formats = short_form_multiple_formats(formats)
            for alt_path, alt_fmt in new_paired_paths:
                self.paired_notebooks[alt_path] = short_form_one_format(
                    alt_fmt), short_formats
        ...
        def set_default_format_options(self, format_options, read=False):
            ...
            if self.comment_magics is not None:
                format_options.setdefault('comment_magics',
                                          self.comment_magics)
        ...
        def create_prefix_dir(self, path, fmt):
            ...
            create_prefix_dir_from_path(self._get_os_path(path.strip('/')),
                                        fmt)
        ...
        def save(self, model, path=''):
            ...
            try:
                ...
                jupytext_formats = jupytext_metadata.get(
                    'formats') or self.default_formats(path)
                ...
                if not jupytext_formats:
                    ...
                    if ext == text_representation.get(
                            'extension') and text_representation.get(
                                'format_name'):
                        fmt['format_name'] = text_representation.get(
                            'format_name')
                ...
                for fmt in jupytext_formats[::-1]:
                    ...
                    value = super(JupytextContentsManager,
                                  self).save(model, alt_path)
                    if alt_path == path:
                        return_value = value
                ...
                for fmt in jupytext_formats[::-1]:
                    ...
                    if 'format_name' in fmt and fmt['extension'] not in [
                        ...
                        self.log.info("Saving %s in format %s:%s",
                                      ...
                                      fmt['extension'][1:], fmt['format_name'])
                    else:
                        self.log.info("Saving %s", os.path.basename(alt_path))
        ...
        def get(self,
            ...
            # Not a notebook?
            if (not self.exists(path) or os.path.isdir(os_path)
                    or (type != 'notebook'
                        if type else ext not in self.all_nb_extensions())):
                return super(JupytextContentsManager,
            ...
            if not content:
                ...
                for alt_path, _ in paired_paths(path, fmt, formats):
                    if alt_path != path and self.exists(alt_path):
                        alt_model = self._notebook_model(alt_path,
                                                         content=False)
            ...
            nbk = model['content']
            jupytext_formats = nbk.metadata.get(
                'jupytext', {}).get('formats') or self.default_formats(path)
            jupytext_formats = long_form_multiple_formats(
                jupytext_formats,
                nbk.metadata,
                auto_ext_requires_language_info=False)
            ...
            if jupytext_formats:
                try:
                    ...
                    self.log.info("Unable to read paired notebook: %s",
                                  str(err))
            else:
            ...
            if path.endswith('.ipynb'):
                ...
                for alt_path, alt_fmt in alt_paths:
                    if not alt_path.endswith('.ipynb') and self.exists(
                            alt_path):
                        source_timestamps[alt_path] = self._notebook_model(
                            alt_path, content=False)['last_modified']
                ...
                for alt_path in source_timestamps:
                    ...
                    if len(source_timestamps) > 1:
                        self.log.info(
                            u'File {} was last modified at {}'.format(
                                alt_path, alt_ts))
                ...
                if most_recent_timestamp is not None:
                    self.log.info(
                        u'Reading SOURCE from {}'.format(path_inputs))
                    model = self.get(path_inputs,
                                     content=content,
                                     type='notebook',
                                     format=format,
            ...
            # Outputs taken from ipynb if in group, if file exists
                ...
                for alt_path, _ in alt_paths:
                    if alt_path.endswith('.ipynb') and self.exists(alt_path):
                        self.log.info(
                            u'Reading OUTPUTS from {}'.format(alt_path))
                        path_outputs = alt_path
                        model_outputs = self.get(alt_path,
                                                 content=content,
                                                 type='notebook',
                                                 format=format,
                                                 load_alternative_format=False)
            ...
            for cell in model['content'].cells:
                if cell.cell_type == 'code' and not cell.outputs and cell.metadata.get(
                        'trusted') is False:
        ...
        def trust_notebook(self, path):
            ...
            for alt_path, alt_fmt in paired_paths(path, fmt, formats):
                if alt_fmt['extension'] == '.ipynb':
                    super(JupytextContentsManager,
                          self).trust_notebook(alt_path)
        ...
        def rename_file(self, old_path, new_path):
            ...
            for old_alt_path, alt_fmt in old_alt_paths:
                ...
                if self.exists(old_alt_path):
                    super(JupytextContentsManager,
                          self).rename_file(old_alt_path, new_alt_path)
