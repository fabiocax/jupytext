...
def str2bool(value):
    ...
    raise argparse.ArgumentTypeError(
        'Expected: (Y)es/(T)rue/(N)o/(F)alse/(D)efault')
...
def parse_jupytext_args(args=None):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "- ext is one of {}, or auto\n".format(', '.join(
                ext[1:] for ext in NOTEBOOK_EXTENSIONS)) +
            "- format_name is optional, and can be {} for Markdown\nfiles, and "
            .format(' or '.join(
                set(fmt.format_name
                    for fmt in JUPYTEXT_FORMATS if fmt.extension == '.md'))) +
            "{} for scripts.\n".format(', '.join(
                set(fmt.format_name
                    for fmt in JUPYTEXT_FORMATS if fmt.extension == '.py'))) +
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=
        'In --test mode, stop on first round trip conversion error, and report\n'
        'stack traceback')
    ...
    parser.add_argument(
        ...
        action='append',
        help=
        'Pipe the text representation (in format --pipe-fmt) of the notebook into\n'
    ...
    parser.add_argument(
        ...
        help=
        'The format in which the notebook should be piped to other programs,\n'
...
def jupytext(args=None):
    ...
    if (args.test or args.test_strict
        ) and not args.to and not args.output and not args.sync:
...
def set_format_options(fmt, format_options):
    ...
    for opt in format_options:
        try:
            ...
            raise ValueError(
                "Format options are expected to be of the form key=value, not '{}'"
                .format(opt))

        if key not in _VALID_FORMAT_OPTIONS:
            raise ValueError(
                "'{}' is not a valid format option. Expected one of '{}'".
...
def load_paired_notebook(notebook, fmt, nb_file, log):
    ...
    if not formats:
        raise NotAPairedNotebook(
            "'{}' is not a paired notebook".format(nb_file))
    ...
    if latest_outputs and latest_outputs != latest_inputs:
        ...
        inputs = notebook if latest_inputs == nb_file else read(latest_inputs,
                                                                fmt=input_fmt)
        check_file_version(inputs, latest_inputs, latest_outputs)
        log("[jupytext] Loading output cells from '{}'".format(latest_outputs))
        outputs = notebook if latest_outputs == nb_file else read(
            latest_outputs)
