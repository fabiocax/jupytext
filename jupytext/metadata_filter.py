...
def metadata_filter_as_dict(metadata_config):
    ...
    for key in metadata_keys:
        ...
        if key.startswith('-'):
            ...
            metadata_config.setdefault('additional',
                                       []).append(key[1:].strip())
        else:
    ...
    for section in metadata_config:
        if 'all' in metadata_config[section]:
            ...
            metadata_config[section] = [
                k for k in metadata_config[section] if k
            ]
...
def metadata_filter_as_string(metadata_filter):
    ...
    if additional == 'all':
        ...
        entries = [
            key for key in additional if key not in _JUPYTEXT_CELL_METADATA
        ]
...
def update_metadata_filters(metadata, jupyter_md, cell_metadata):
    ...
    if 'cell_metadata_filter' in metadata.get('jupytext', {}):
        ...
        metadata.setdefault(
            'jupytext',
            {})['cell_metadata_filter'] = metadata_filter_as_string(
                metadata_filter)
    ...
    if not jupyter_md:
        ...
        nb_md_filter = metadata.get('jupytext',
        ...
        for key in metadata:
            if key in _DEFAULT_NOTEBOOK_METADATA.split(
                    ',') or key in nb_md_filter or ('-' + key) in nb_md_filter:
                continue
            nb_md_filter.append(key)
        if nb_md_filter:
            metadata.setdefault(
                'jupytext',
                {})['notebook_metadata_filter'] = ','.join(nb_md_filter)
...
def filter_metadata(metadata, user_filter, default_filter=''):
    ...
    if isinstance(default_include,
                  list) and default_include and default_exclude == []:
        default_exclude = 'all'
    ...
    return subset_metadata(metadata,
                           exclude=set(user_exclude).union(
                               set(default_exclude).difference(user_include)))
...
def subset_metadata(metadata, keep_only=None, exclude=None):
    ...
    if keep_only is not None:
        filtered_metadata = {
            key: metadata[key]
            for key in metadata if key in keep_only
        }
...
def restore_filtered_metadata(filtered_metadata, unfiltered_metadata,
                              user_filter, default_filter):
    """Update the filtered metadata with the part of the unfiltered one that matches the filter"""
    filtered_unfiltered_metadata = filter_metadata(unfiltered_metadata,
                                                   user_filter, default_filter)
